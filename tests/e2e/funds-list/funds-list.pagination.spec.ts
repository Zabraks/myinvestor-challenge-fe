...
/**
 * Tests de Paginación - Listado de Fondos
 *
 */
test.describe('Funds List - Pagination', () => {
  ...
  test('should show correct number of items on last page', async ({ fundsPage }) => {
    await fundsPage.getByRole('link', { name: SELECTORS.pagination.next }).click();
    await fundsPage.getByRole('link', { name: SELECTORS.pagination.next }).click();
  ...
  test('should change items per page using the limit selector', async ({ fundsPage }) => {
    ...
    const limitSelector = fundsPage.getByRole('combobox');
    ...
    await expect(limitSelector).toContainText('25');
  });
